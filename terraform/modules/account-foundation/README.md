...
3. **A GitHub Actions OIDC identity provider registered in this AWS account's IAM** (`https://token.actions.githubusercontent.com`), plus the actual IAM roles that assume it (one for plan, one for apply — scoped by GitHub repo/branch condition in their trust policy). **This module does not create the OIDC provider or those roles** — it only creates the roles that `plan_role_trusted_principal_arns`/`apply_role_trusted_principal_arns` name as trusted principals, i.e. it grants state-bucket permissions to roles that must already exist. That's [`terraform/modules/github-oidc`](../github-oidc/README.md) — a separate module built specifically for this, wired ahead of `account-foundation` via a Terragrunt `dependency` block (see `terragrunt/README.md`).
